...
export const generateModelImage = async (userImages: File[], measurements?: BodyMeasurements): Promise<string> => {
    const userImageParts = await Promise.all(userImages.map(file => fileToPart(file)));
    
    let measurementText = "";
    if (measurements) {
        measurementText = `The model should have the following body measurements: 
        ...
        ${measurements.bodyType ? `Body Type: ${measurements.bodyType}` : ""}`;
    }

    const prompt = `You are an expert fashion photographer AI. Transform the person in these images into a single, full-body fashion model photo suitable for an e-commerce website. 
    ${userImageParts.length > 1 ? "Use the multiple provided images to accurately capture the person's facial features and identity from different angles." : ""}
    ${measurementText}
