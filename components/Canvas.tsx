...
import { RotateCcwIcon, ChevronLeftIcon, ChevronRightIcon, BoxIcon, HeartIcon, CameraIcon } from './icons';
...
const Canvas: React.FC<CanvasProps> = ({ 
  ...
  const rotationAvailable = useMemo(() => {
    return rotationIndices.every(idx => availablePoseKeys.includes(poseInstructions[idx]));
  }, [rotationIndices, availablePoseKeys, poseInstructions]);

  const currentRotationIndex = useMemo(() => {
    return rotationIndices.indexOf(currentPoseIndex);
  }, [rotationIndices, currentPoseIndex]);

  const handleRotationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const idx = parseInt(e.target.value);
    onSelectPose(rotationIndices[idx]);
  };
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      ...
      <div className="absolute top-4 left-4 z-30 flex flex-col gap-3">
        <button 
            onClick={onStartOver}
            className="flex items-center justify-center text-center bg-white/60 border border-gray-300/80 text-gray-700 font-semibold py-2 px-4 rounded-full transition-all duration-200 ease-in-out hover:bg-white hover:border-gray-400 active:scale-95 text-sm backdrop-blur-sm shadow-sm"
        >
        ...
        {displayImageUrl && !isLoading && (
          <>
            <button 
                ...
                className={`flex items-center justify-center text-center font-semibold py-2 px-4 rounded-full transition-all duration-200 ease-in-out active:scale-95 text-sm backdrop-blur-sm shadow-sm border ${
                  isSaved 
      ...
      {/* 360 Spin Button */}
      {displayImageUrl && !isLoading && !rotationAvailable && (
        <button 
            onClick={onGenerate360}
            className="absolute top-4 right-4 z-30 flex items-center justify-center text-center bg-gray-900 text-white font-semibold py-2 px-4 rounded-full transition-all duration-200 ease-in-out hover:bg-gray-700 active:scale-95 text-sm shadow-lg"
        >
      ...
      {displayImageUrl && !isLoading && rotationAvailable && (
        <div className="absolute top-4 right-4 z-30 flex flex-col items-end gap-2">
          <div className="bg-white/80 backdrop-blur-md border border-gray-200 p-3 rounded-xl shadow-lg flex flex-col gap-2 w-48">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">360 View</span>
              <BoxIcon className="w-4 h-4 text-gray-400" />
            </div>
            <input 
              type="range" 
              min="0" 
              max={rotationIndices.length - 1} 
              step="1" 
              value={currentRotationIndex !== -1 ? currentRotationIndex : 0}
